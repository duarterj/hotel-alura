...
public class RegistroHospede extends JFrame {
	...
	public RegistroHospede(Integer id) {
		...
		inicializaRegistroHospede(id);
	}

	private void inicializaRegistroHospede(Integer id) {
		int numeroReserva = id;
		setIconImage(Toolkit.getDefaultToolkit().getImage(RegistroHospede.class.getResource("/resources/imagens/lOGO-50PX.png")));
		...
		txtDataN.getCalendarButton().setIcon(new ImageIcon(RegistroHospede.class.getResource("/resources/imagens/icon-reservas.png")));
		...
		txtNreserva.setFont(new Font("Roboto", Font.BOLD, 20));
		...
		btnSalvar.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				Boolean nomeEstaVazio = txtNome.getText().isEmpty();
				Boolean sobrenomeEstaVazio = txtSobrenome.getText().isEmpty();

				if (nomeEstaVazio == false && sobrenomeEstaVazio == false && txtDataN.getDate() != null) {
					salvarHospede();
					Sucesso sucesso = new Sucesso();
					sucesso.setVisible(true);
					dispose();
				} else {
					JOptionPane.showMessageDialog(contentPane, "Preencha os campos obrigatórios!");
				}
			}
		});
		...
		imageFundo.setIcon(new ImageIcon(RegistroHospede.class.getResource("/resources/imagens/registro.png")));

		JLabel logo = new JLabel("");
		logo.setBounds(194, 39, 104, 107);
		panel.add(logo);
		logo.setIcon(new ImageIcon(RegistroHospede.class.getResource("/resources/imagens/Ha-100px.png")));
	}
	...
	private void salvarHospede() {
		...
		Hospede novoHospede = new Hospede(txtNome.getText(), txtSobrenome.getText(), java.sql.Date.valueOf(dataNascimento), txtNacionalidade.getSelectedItem().toString(), txtTelefone.getText(), numeroReserva);
		hospedesController.salvar(novoHospede);
	}
